...
def is_moderator(user_id: int) -> bool:
    ...
    try:
        if USE_POSTGRES:
            cur = conn.cursor()
            cur.execute("SELECT 1 FROM moderators WHERE user_id = %s", (user_id,))
            row = cur.fetchone()
            cur.close()
        else:
...
def db_add_moderator(user_id: int, added_by: int) -> bool:
    ...
    try:
        if USE_POSTGRES:
            ...
            conn.execute("INSERT OR IGNORE INTO moderators (user_id, added_by) VALUES (?, ?)", (user_id, added_by))
            conn.commit()
        return True
    except Exception as e:
        logger.error(f"Add moderator error: {e}")
        return False
    finally:
        put_db_conn(conn)
...
def db_remove_moderator(user_id: int) -> bool:
    ...
    try:
        if USE_POSTGRES:
            cur = conn.cursor()
            cur.execute("DELETE FROM moderators WHERE user_id = %s", (user_id,))
            conn.commit()
            cur.close()
        else:
        ...
        logger.error(f"Remove moderator error: {e}")
        return False
    finally:
        put_db_conn(conn)
...
def get_moderators():
    ...
    try:
        if USE_POSTGRES:
            cur = conn.cursor()
            cur.execute("SELECT user_id, added_by, added_at FROM moderators ORDER BY added_at")
            rows = cur.fetchall()
            cur.close()
            return [{"user_id": r[0], "added_by": r[1], "added_at": r[2]} for r in rows]
        else:
...
async def get_telegram_logs(auth=Depends(verify_auth), user_id: int = None, username: str = None):
    conn = get_db_conn()
    try:
        if USE_POSTGRES:
            cur = conn.cursor()
...
async def get_web_logs(auth=Depends(verify_auth), ip: str = None):
    conn = get_db_conn()
    try:
        if USE_POSTGRES:
            cur = conn.cursor()
...
def can_manage_whitelist(update: Update) -> bool:
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(update):
        ...
        log_telegram(user.id, user.username, "/stats", "Нет прав")
        return
    conn = get_db_conn()
    try:
        if USE_POSTGRES:
...
async def logs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(update):
        await update.message.reply_text("❌ Нет прав.")
        log_telegram(user.id, user.username, "/logs", "Нет прав")
        return
    limit = 10
...
async def web_logs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(update):
        await update.message.reply_text("❌ Нет прав.")
        log_telegram(user.id, user.username, "/web_logs", "Нет прав")
        return
    limit = 10
    if context.args and context.args[0].isdigit():
        limit = int(context.args[0])
...
async def user_logs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(update):
        await update.message.reply_text("❌ Нет прав.")
        log_telegram(user.id, user.username, "/user_logs", "Нет прав")
        return
    if not context.args:
...
async def broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(update):
        await update.message.reply_text("❌ Нет прав.")
        log_telegram(user.id, user.username, "/broadcast", "Нет прав")
        return
    if not context.args:
...
async def whitelist_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    if not can_manage_whitelist(update):
        await update.message.reply_text("❌ Нет прав.")
        log_telegram(user.id, user.username, "/whitelist", "Нет прав")
        return
    users = get_whitelist()
    if not users:
...
async def whitelist_add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not can_manage_whitelist(update):
        await update.message.reply_text("❌ Нет прав.")
        log_telegram(user.id, user.username, "/whitelist_add", "Нет прав")
        return
    if not context.args:
    ...
    if add_to_whitelist(uid, user.id):
        ...
        log_telegram(user.id, user.username, f"/whitelist_add {uid}", "Ошибка БД")

async def whitelist_remove(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    if not can_manage_whitelist(update):
        await update.message.reply_text("❌ Нет прав.")
        log_telegram(user.id, user.username, "/whitelist_remove", "Нет прав")
        return
    if not context.args:
    ...
    if remove_from_whitelist(uid):
        ...
        log_telegram(user.id, user.username, f"/whitelist_remove {uid}", "OK")
    else:
        await update.message.reply_text("❌ Ошибка при удалении.")
        log_telegram(user.id, user.username, f"/whitelist_remove {uid}", "Ошибка БД")

async def make_moderator(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    if not is_admin(update):
        await update.message.reply_text("❌ Только главный администратор может назначать модераторов.")
        log_telegram(user.id, user.username, "/make_moderator", "Нет прав")
        return
    if not context.args:
    ...
    if db_add_moderator(uid, user.id):
        ...
        await update.message.reply_text("❌ Ошибка при назначении модератора.")
        log_telegram(user.id, user.username, f"/make_moderator {uid}", "Ошибка БД")

async def remove_moderator(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    if not is_admin(update):
        await update.message.reply_text("❌ Только главный администратор может удалять модераторов.")
        log_telegram(user.id, user.username, "/remove_moderator", "Нет прав")
        return
    if not context.args:
...
async def list_moderators(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(update):
        await update.message.reply_text("❌ Нет прав.")
        log_telegram(user.id, user.username, "/list_moderators", "Нет прав")
        return
    mods = get_moderators()
    if not mods:
...
async def list_commands(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(update):
        await update.message.reply_text("❌ Нет прав.")
        log_telegram(user.id, user.username, "/list", "Нет прав")
        return
    msg = (
        ...
        "/user_logs <ID> – Логи пользователя\n"
        "/broadcast <текст> – Массовая рассылка\n"
        "/whitelist – Показать белый список\n"
        "/whitelist_add <ID> – Добавить в белый список (админ/модератор)\n"
        "/whitelist_remove <ID> – Удалить из белого списка (админ/модератор)\n"
...
async def startup():
    ...
    if ADMIN_USER_ID:
        ...
        bot_app.add_handler(CommandHandler("make_moderator", make_moderator))
        bot_app.add_handler(CommandHandler("remove_moderator", remove_moderator))
